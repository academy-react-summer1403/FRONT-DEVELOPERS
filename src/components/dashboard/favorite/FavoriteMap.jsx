...
const FavoriteMap = ({ search, category }) => {
  ...
  const handleRemoveFavorite = (userFavoriteId) => {
    ...
    deleteFavoriteCourse(CourseFavoriteId).then(() => {
      setFavoriteCourse((prevData) =>
        prevData.filter((item) => item.favoriteId !== userFavoriteId)
      );
    });
  };
  ...
  const filteredData = favoriteCourse.filter((f) => {
    ...
    const matchesCategory =
      category === "همه" || f.levelName.includes(category);
