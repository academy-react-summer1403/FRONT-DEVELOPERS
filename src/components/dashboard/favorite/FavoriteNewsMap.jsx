...
const FavoriteMapNew = ({ search }) => {
  ...
  const [favoriteNew, setfavoriteNew] = useState(
    favoriteNews?.data?.myFavoriteNews || []
  );
  useEffect(() => {
  ...
  const handleRemoveFavoriteNews = (deleteFavorite) => {
    ...
    deleteFavoriteNews(params).then(() => {
      setfavoriteNew((prevData) =>
        prevData.filter((item) => item?.favoriteId !== deleteFavorite)
      );
    });
  };
  ...
  const filteredData = favoriteNew.filter(
    (f) =>
      search.trim() === "" ||
          ...
          text-gray-600 dark:text-white font-medium justify-items-center "
            >
              <li className="col-1 my-2 flex gap-2">
                <form>
                  <button
                    type="button"
                    className=""
                    onClick={() => setDeletmodal(item?.newsId)}
                  >
                ...
                <div
                  className={`${
                    deletmodal == item?.newsId ? "block" : "hidden"
                ...
                >
                  <div className="bg-white rounded-lg shadow-lg grid gap-3 p-8 mx-auto w-1/3 mt-40">
                    ...
                    <button
                      to={"#"}
                      onClick={() => (
                        handleRemoveFavoriteNews(item?.favoriteId),
                        setDeletmodal(false)
                      )}
                ...
                <div>
                  <Link to={"/article-detail/" + item?.newsId}>
                    {" "}
              ...
              <li className="col-1 my-5">
                <DateApi dateapi={item?.updateDate} />{" "}
              </li>
              <li className="col-1 my-5">--</li>
