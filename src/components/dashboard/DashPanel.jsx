...
const DashPanel = () => {
  ...
  return (
    <div
      className="bg-gray-50 dark:bg-gray-900 py-4 px-5 h-screen
      ...
      >
        ...
        <div
          className={`right-0 bg-primary font-red-500 rounded-r-xl py-[8px] z-40 relative
            ...
            max-sm:w-full max-sm:rounded-t-xl max-sm:rounded-br-none max-sm:${
              resposive ? "h-[35%]" : "h-2"
            } 
            `}
        >
          {/* responsive icon  */}
          <IoIosMenu
            onClick={() => setResposive(HandeleResposive)}
            className={`hidden w-6 h-6 hover:cursor-pointer text-teal-900
          ...
          {/* profile section: */}
          <div
            className={`border-b-[1.5px] border-gray-200/60 mx-4 py-4
                    max-sm:mx-0 max-sm:block
                    max-lg:${resposive ? "block" : "hidden"}
                `}
          >
            <img
              src={userProfile.data?.currentPictureAddress}
              onError={ImageErrore}
            ...
            />
            <p
              className="mt-4 text-center text-darkgreen font-bold
                        max-lg:text-sm 
                    "
            >
          ...
          >
            <div
              className={`max-lg:${resposive ? "block" : "hidden"} max-sm:${
                resposive ? "block" : "hidden"
              } `}
            >
              خروج از حساب
            </div>
