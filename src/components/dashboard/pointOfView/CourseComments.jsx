      ...
      {paginatedData.map((item, i) => (
        ...
        >
          <li className="col-1 my-2 flex gap-2">
            <div>
              <TbTrash
                onClick={() => setDeletmodal(item?.commentId)}
            ...
            <div
              className={`${
                deletmodal == item?.commentId ? "block" : "hidden"
            ...
            >
              <div className="bg-white rounded-lg shadow-lg grid gap-3 p-8 mx-auto w-1/3 mt-40">
                ...
                <p className="dark:text-gray-950 text-[20px] text-center">
                  {" "}
                  آیا میخواهید دیگاه را حذف کنید؟{" "}
                </p>
                <button
                  to={"#"}
                  onClick={() => (
                    HandleDeleteSubmit(item.commentId), setDeletmodal(false)
                  )}
          ...
          <li className="col-1 leading-[50px]">#00{i}</li>
        </ul>
      ))}
