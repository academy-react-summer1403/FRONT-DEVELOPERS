...
function AddProfImage({ allimages, currentImage }) {
  ...
  const handlePostUserImg = (image) => {
    ...
    postUserImg(formFile).then(
      toast.success("عکس با موفقیت اضافه شد", { theme: "colored" })
    );
  };
  ...
  const handleChoose = (dataId) => {
    ...
    postUserMainImg(formId).then(
      toast.success("عکس با موفقیت انتخاب شد", { theme: "colored" })
    );
  };
  ...
  return (
    <div>
      ...
      >
        <img
          src={currentImage ? currentImage : <ImageErrore />}
          onError={ImageErrore}
      ...
      <div
        style={{
          backdropFilter: "box-shadow: 0px 2px 4px 0px rgba(0, 0, 0, 0.25) ",
        }}
        className={` ${
          !webcam && Open ? "block" : "hidden "
        } fixed top-[100px] w-[58%] h-[75%] flex flex-col justify-center gap-14 items-center left-[250px] bg-neutral-500/80 z-[9999] rounded-xl
      ...
      >
        ...
        <div className="relative flex">
          <img
            src={currentImage ? currentImage : <ImageErrore />}
            onError={ImageErrore}
            className="w-56 h-56 rounded-xl  bg-gray-100"
          />
          <div
            onClick={() => handlePostUserImg(preview)}
            data-tooltip-id="tooltip"
            data-tooltip-content="تنظیم به عنوان تصویر اصلی"
            className="cursor-pointer w-9 h-9 absolute pl-1.5 pt-1.5 bg-white rounded-full shadow-md bottom-[-10px] right-[-10px]"
          >
        ...
        <div className="flex flex-row gap-3 justify-center text-white">
          {allimages?.map((data, index) => (
            <div key={index} className="relative group">
              <img
                ...
                src={data.puctureAddress}
                className="w-[100px] h-[100px] rounded-md"
              />
              <div
                onClick={() => handleDeleteImage(data.id)}
                className="cursor-pointer w-6 h-6 absolute hidden group-hover:block rounded-full bg-white text-red-500 p-1 -top-2 -left-2"
              >
                <HiX />
              </div>
            </div>
          ...
          <div
            ...
            data-tooltip-content="اضافه کردن عکس جدید"
          >
            <Avatar
              width={100}
              height={100}
          ...
          <div
            ...
            className="w-[100px] h-[100px] border-dashed border-2 rounded-md border-gray-200/20 text-sm text-center pt-10 font-semibold cursor-pointer"
          >
            webcam
          </div>
        </div>
      ...
      >
        ...
        <div className="relative flex">
          <div
            onClick={() => (
              handlePostUserImg(preview), setOpen(false), setWebcam(false)
            )}
            data-tooltip-id="tooltip"
          ...
          <Tooltip id="tooltip" />
          {webcam && (
            <Camera setPreview={setPreview} base64ToFile={base64ToFile} />
          )}
        </div>
      </div>
    </div>
