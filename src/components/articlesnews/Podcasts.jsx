      ...
      return (
        ...
        >
                <div className='text-center '>
                    <h3 className='text-lg font-medium text-darkgreen dark:text-white mb-2'>{t("podcasts")}</h3>
                    <p className='text-sm text-gray-400 dark:text-gray-300'>{t("podcasts")}</p>
                </div>        
                        {newsDataQuery.data?.news.map((item,key)=>(
                            <NavLink to={"/article-detail/" + item.id} key={key} className='relative flex flex-row my-4'>
                                <img data-aos="flip-right" data-aos-duration="700" src={react} className='z-[9999]'/>
                                <div 
                                ...
                                className='absolute bg-gray-100 dark:bg-gray-500/70 w-[82%] right-0 my-2 px-2 py-4 text-right shadow rounded-lg
                                    ...
                                    max-lg:w-[92%] 
                                '>
                                    <p className='text-[11px] font-medium text-darkgreen/80 dark:text-white mb-3'>{item?.title}</p>
                                    <div  className='text-[10px] text-gray-400 flex flex-row px-[25%] max-lg:px-[20%] max-sm:px-[10%]  gap-5 '>
                                        <p className='flex flex-row-reverse gap-1 '> 
                                           {item?.currentView}<img src={view}/>
                                        </p> 
                                        <p className='flex flex-row-reverse gap-1'>
                                            {item?.currentLikeCount}<img src={likes}/>
                                        </p> 
                                    </div>
                                </div>
                            </NavLink>
