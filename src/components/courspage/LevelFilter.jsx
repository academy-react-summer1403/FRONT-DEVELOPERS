    ...
    <>
      <div className="z-40 relative flex flex-col  gap-2 ">
        <div className="flex flex-row justify-end">
          <label className="cursor-pointer" htmlFor="level1" to="checkbox">
            {" "}
            مبتدی
          </label>
          <input
            ...
            onClick={
              filter
                ? () => dispatch(Add(Levels.data?.[0].id), setFilter(false))
                : () => dispatch(Add(""), setFilter(true))
            }
          />
        </div>
        <div className="flex flex-row justify-end -ml-[3px]">
          <label className="cursor-pointer" htmlFor="level2" to="checkbox">
            متوسط
          </label>
          <input
            id="level2"
            ...
            onClick={
              filter2
                ? () => dispatch(Add(Levels.data?.[1].id), setFilter(false))
                : () => dispatch(Add(""), setFilter2(true))
            }
          />
        </div>
        <div className=" flex flex-row justify-end">
          <label className="cursor-pointer" htmlFor="level3" to="checkbox">
            پیشرفته
          </label>
          <input
            id="level3"
            ...
            onClick={
              filter3
                ? () => dispatch(Add(Levels.data?.[2].id), setFilter(false))
