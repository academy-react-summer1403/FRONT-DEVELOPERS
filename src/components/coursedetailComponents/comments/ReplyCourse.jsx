...
import { useDeleteCourseCommentLike, useDislikeCourseComment, useLikeCommentCourse } from '../../../core/services/mutation/DetailsMutation'

    
const ReplyCourse = ({courseId , courseCommmentId  , showMore}) => {

    const getReply = useReplyCourse( courseId , courseCommmentId)
    console.log("getReply" , getReply.data)


    const [like, setLike] = useState(getReply.data)
    useEffect(() => {
        if(getReply.data){
            setLike(getReply.data)
...
//              DeleteCourseCommentLike.mutate( likeid);
            ...
            >  
                ...
                <div className=' border-b border-gray-300 w-full justify-between h-[80px] flex items-center'>
                    ...
                    <div  className='  gap-3 flex items-center justify-between'>
                        {/* AddReply:  */}
                        <svg className='cursor-pointer' onClick={()=>handleReply(courseCommmentId)} width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <AddReplyCourse  comentdiv={replydiv} setCommentdiv={setReplydiv} holder={"افزودن ریپلای"} Id={courseId} commentId={courseCommmentId}/> 
                           ...
                           <h3 className=' like flex items-center gap-2'>
                            ...
                            <h3 onClick={()=>handleDissLikeComment(item?.id)}  className='like flex items-center gap-2'>            
                                <svg onClick={()=>""}width="20"height="19" 
                                className={`cursor-pointer max-lg:w-[15px] max-lg:h-[17px] stroke-orange/80
                                    ${item?.currentUserEmotion == "DISSLIKED" ? "fill-orange/80 " : " "} `}viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
