...
const CourseComment = ({courseId}) => {
  ...
  const [comentdiv1,setCommentdiv1]=useState(false)
...
<h3  onClick={()=>setLike(item?.id)} className=' like flex items-center gap-2'>
    <svg  onClick={()=>(setlike1(!like1),setDislike1(dislike1==false ? !dislike1 : dislike1))} width="20" height="19" className={` max-lg:w-[15px] max-lg:h-[17px] stroke-orange/80
      ${like1 ? "fill-none" : "fill-orange/80"} `}viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{item?.likeCount}</h3>
 <h3 onClick={()=>setLike(item?.id)}  className=' like flex items-center gap-2'>            
              <svg onClick={()=>(setDislike1(!dislike1),setlike1(like1==false ? !like1 : like1))}width="20"height="19"className={` max-lg:w-[15px] max-lg:h-[17px] stroke-orange/80
              ${dislike1 ? "fill-none" : "fill-orange/80"} `}viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
