...
import { useReplyNews } from '../../../core/services/query/CommentQuery'
...
const ReplyArticle = ({showMore , Id , UserId}) => {
    const getReply = useReplyNews( Id)
console.log(getReply.data)
    ...
    const handleLikeComment =(id)=>{
        ...
        getReply.data?.map((comment)=>{
            if(comment.id === id){
                postLikeComment.mutate( likeparams);
                comment.currentUserIsLike = !comment.currentUserIsLike ;
                comment.likeCount +=  1 ;                
            }
        })       
    }
    ...
    // delete like :
    // const DeleteLikeComment =useDeleteLikeComment()
    // const handledeleteComment =(likeid)=>{
    //     const params={
    //         "deleteEntityId": likeid
    //     }
    //     getReply.data?.map((comment)=>{
    //         if(comment.currentUserLikeId === likeid){
    //             DeleteLikeComment.mutate( params);
    //             comment.currentUserIsLike = !comment.currentUserIsLike ;
    ...
    //     GetCommentNews.data?.map((comment)=>{
    //         if(comment.id === id){
    //             comment.currentUserIsDissLike = !comment.currentUserIsDissLike ;
    //             comment.dissLikeCount +=  comment.dissLikeCount ? 1 : -1 ;                
    //         }
    //     })       
    // }
  ...
  return (
    ...
    {loading ? <CommentsLoading/> :
        getReply.data?.map((item)=>(
            <div style={{boxShadow:"0px 1px 1px 0px rgba(0 ,0,0,0.2) inset"}}
             className=' bg-[#ECECEC] dark:bg-slate-900  px-5 h-[150px] mb-3  rounded-[10px]  w-full flex  flex-wrap border-r-4 border-primary'
            >  

                <div className=' border-b border-gray-300 w-full justify-between h-[80px] flex items-center'>

                    <div  className='  gap-3 flex items-center justify-between'>
                        {/* AddReply:  */}
                        <svg className='cursor-pointer' onClick={()=>handleReply(Id)} width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <AddReplyNews  comentdiv={replydiv} setCommentdiv={setReplydiv} Id={Id} UserId={UserId} newsId={item?.newsId}/> 

                           {/* comment like: */}

                           <h3 className=' like flex items-center gap-2'>
                                <svg  onClick={()=>{item?.currentUserIsLike == true  ? handledeleteComment("")  : handleLikeComment(item?.id)}} width="20" height="19" className={`cursor-pointer max-lg:w-[15px] max-lg:h-[17px] stroke-orange/80
                                ${item?.currentUserIsLike == true ? "fill-orange/80 " : " "} `}viewBox="0 0 20 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <div className='  flex items-center'>                  
                        ...
                        <img className=' profile' src={item?.pictureAddress ? item?.pictureAddress : ImageErrore} onError={ImageErrore}  />
                    </div>
