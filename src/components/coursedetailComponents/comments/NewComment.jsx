    ...
    <div
      ...
      className={`mt-[15px] bg-white dark:bg-slate-700 rounded-[15px] p-[20.5px] 
       overflow-hidden relative ${
         showMore && GetCommentNews?.data?.length !== 0
           ? "mb-40 h-[770px]"
           : GetCommentNews?.data?.length === 0
           ? " h-[470px]"
           : "min-h-[770px]"
       }`}
    >
      ...
      {loading ? (
        ...
        <h1 className="text-3xl text-green py-24 my-12  text-center">
          ... نظری وجود ندارد
        </h1>
      ) : (
                    ...
                    <svg
                      onClick={() => {
                        item?.currentUserIsLike == true
                          ? handledeleteComment(item?.currentUserLikeId)
