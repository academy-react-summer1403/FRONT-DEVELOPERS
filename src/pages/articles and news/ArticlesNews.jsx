...
const ArticlesNews = () => {
  ...
  const filteredNews = isfillter.length
    ? newsDataQuery.data?.news.filter(
        (f) =>
          f.miniDescribe.includes(isfillter[0]) ||
          f.miniDescribe.includes(isfillter[1])
      )
        ...
        <div className="relative col-span-3 ">
          ...
          >
            ...
            <div className="relative group max-sm:grid max-sm:justify-items-center">
              <div
                className="group flex px-5 py-2 mr-4 gap-6 dark:bg-gray-700/70 rounded-full shadow-sm shadow-gray-300 outline-none indent-0
            ...
            <div className="relative max-sm:grid max-sm:justify-items-center">
              <div
                className="grid grid-cols-3 gap-4 mx-auto justify-items-center 
                ...
                {filter.map((item) => (
                  <div
                    ...
                    className={`relative flex ${
                      isfillter[0] === item.tool1
                        ? "border-b border-darkgreen"
                        : ""
                    }`}
                    data-tooltip-id="tooltip"
                    data-tooltip-content={`${item.tool1}  ${item.tool2}`}
                  >
                    <p onClick={() => setIsfillter([item.tool1, item.tool2])}>
                      {item.icon}
                    </p>
                    <Tooltip id="tooltip" />
                  </div>
